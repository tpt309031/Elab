        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-3">{selection.map((row) => {
              ...
              return <div key={String(row.model)} className="space-y-2 border-b border-border pb-3 last:border-0"><div className="flex items-center justify-between gap-3"><span className="flex items-center gap-2 text-sm">{active ? <CheckCircle2 className="size-4 text-emerald-400" /> : <CircleOff className="size-4 text-muted-foreground" />}{String(row.model)}</span><Badge variant={active ? "default" : "secondary"}>{String(row.status)}</Badge></div><Progress value={Number(row.weight) * 100} /><div className="flex justify-between font-mono text-[10px] text-muted-foreground"><span>cal score {(Number(row.calibration_score) * 100).toFixed(1)}%</span><span>weight {(Number(row.weight) * 100).toFixed(1)}%</span></div></div>;
            })}</CardContent>
          </Card>
          <Card>
